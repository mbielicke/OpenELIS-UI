...
public class QueryUtil {
    ...
    public <T extends Comparable<? super T>> Predicate createPredicate(Path<? extends T> path, QueryData qd) {
        ...
        while (fieldCompIt.hasNext()) {
            ...
            switch (comp) {
            	...
            	case "between " :
            		...
            		predicates.add(builder.<T>between(path, getValue(qd.getType(),ends[0]), getValue(qd.getType(),ends[1])));
            		break;
            	case "<" :
            		predicates.add(builder.<T>lessThan(path, getValue(qd.getType(),param)));
            		break;
            	case ">" :
            		predicates.add(builder.<T>greaterThan(path, getValue(qd.getType(),param)));
            		break;
            	case "<=" :
            		predicates.add(builder.<T>lessThanOrEqualTo(path, getValue(qd.getType(),param)));
            		break;
            	case ">=" :
            		predicates.add(builder.<T>greaterThanOrEqualTo(path, getValue(qd.getType(),param)));
            		break;
            	default :
