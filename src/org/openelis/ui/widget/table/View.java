...
public abstract class View<T extends Controller> extends ResizeComposite {
    ...
    protected void cellClick(CellClickedEvent event) {
        if (controller.fireCellClickedEvent(event.getRow(), event.getCol(), event.isCtrlKeyDown(), event.isShiftKeyDown()))
            controller.startEditing(event.getRow(), event.getCol(), event.getNativeEvent());
    }
    ...
    protected void createRow(int row) {
        grid.insertRow(row);
        grid.getRowFormatter().getElement(row).setAttribute("height", (controller.getRowHeight()+3)+"px");
    ...
    public void renderView(int startRow, int endRow) {
        ...
        if (endRow < 0) endRow = controller.getRowCount() - 1;
    ...
    protected void bulkRenderRow(SafeHtmlBuilder builder, int row) {
    	String style = controller.getRowAt(row).getStyle(row);
    	builder.appendHtmlConstant("<tr height='"+(controller.getRowHeight()+3)+"px' index='"+row+"'" + (style != null ? " class='"+style+"'>" : ">"));
    ...
    protected String getExceptionStyle(int r, int c) {
    	return Balloon.isWarning(controller.getEndUserExceptions(r, c), controller.getValidateExceptions(r, c)) ? css.InputWarning() : css.InputError(); 
    }
    ...
    protected void applyRowStyle(int r) {
        ...
        style = controller.getRowAt(r).getStyle(r);
        if (style != null)
    ...
    public void bulkExceptions(HashMap<? extends Row,HashMap<Integer, ArrayList<Exception>>> exceptions) {
        for(Row row : exceptions.keySet()) {
            int r = controller.getModel().indexOf(row);
    ...
    public void startEditing(int r,int c, Object value, NativeEvent event) {
        container.setWidth( (controller.getColumnAt(c).getWidth() - 3));
        container.setHeight( (controller.getRowHeight()));
        ...
        if (controller.getQueryMode())
            controller.getColumnAt(c)
                 .getCellEditor()
                 .startEditingQuery((QueryData)controller.getValueAt(r, c), container, event);
        else
            controller.getColumnAt(c)
                 .getCellEditor()
                 .startEditing(controller.getValueAt(r, c), container, event);
    }
	...
	public Object finishEditing(int r, int c) {
