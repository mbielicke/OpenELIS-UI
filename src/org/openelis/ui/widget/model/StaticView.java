...
import org.openelis.ui.widget.table.Header;
 ...
 * 
 */
public class StaticView extends ResizeComposite {
    ...
    protected void handleClick(ClickEvent event) {
        ...
        //if (table.fireCellClickedEvent(r, c, event.isControlKeyDown(), event.isShiftKeyDown()))
            startEditing(r, c, event.getNativeEvent());
    }
    ...
    protected void layout() {
        ...
        for (int c = 0; c < model.colCount(); c++ ) {
            flexTable.getColumnFormatter().setWidth(c, model.colt(c).getWidth() + "px");
