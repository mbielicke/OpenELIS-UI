...
public class Table<T> extends FocusPanel implements ScreenWidgetInt, Queryable,
		...
		HasUnselectionHandlers<Integer>, HasBeforeCellEditedHandlers,RequiresResize,
		...
		HasValue<ArrayList<T>>, HasExceptions, Focusable, FocusHandler, HasFilterHandlers {
	...
	protected ArrayList<ModelColumn<T,?>> modelCols;
	...
	public Table() {
		...
		modelCols = new ArrayList<ModelColumn<T,?>>();
		view = new View(this);
	    setWidget(view);
	    setKeyHandling();
	}
	...
	public void addModelColumn(ModelColumn<T,?> col) {
		addModelColumnAt(modelCols.size(),col);
	}
	
	public void addModelColumnAt(int index, ModelColumn<T,?> col) {
		modelCols.add(index,col);
	}
	...
	public <V> void setValueAt(int row, int col, V value) {
		...
		((ModelColumn<T,V>)modelCols.get(col)).getFieldUpdater().update(col, getRowAt(row), value);
		
		exceptions = getColumnAt(col).getCellRenderer().validate(value);
		
		if(column.isRequired() && value == null)
			exceptions.add(new Exception(Messages.get().fieldRequired()));
	...
	public <V> V getValueAt(int row, int col) {
		if (modelView == null || row >= modelView.size())
			return null;
		return (V)modelCols.get(col).getValue(getRowAt(row));
	}
	 ...
	 * @return
	 */
	private ArrayList<Exception> getEndUserExceptionList(int row,
			int col) {
		HashMap<Integer, ArrayList<Exception>> cellExceptions = null;
	 ...
	 * @return
	 */
	private ArrayList<Exception> getValidateExceptionList(int row,
			int col) {
		HashMap<Integer, ArrayList<Exception>> cellExceptions = null;
	...
	public HandlerRegistration addValueChangeHandler(
			ValueChangeHandler<ArrayList<T>> handler) {
	...
	private class Sort implements Comparator<T> {
		...
		public int compare(T o1, T o2) {
			...
			return dir;//	* ((Comparable) o1.getCell(col)).compareTo((Comparable) o2
						//	.getCell(col));
		};
	}
    ...
    public void onResize() {
        ...
        Element parent = getParent() instanceof LayoutPanel ? ((LayoutPanel)getParent()).getWidgetContainerElement(this) : getParent().getElement();
        ...
        if(height > 0) {    
           ...
           if(visibleHandler == null) {
               visibleHandler = addVisibleHandler(new VisibleEvent.Handler() {
                   @Override
                   public void onVisibleOrInvisible(VisibleEvent event) {
                       removeVisibleHandler();
                       visibleHandler.removeHandler();
                       visibleHandler = null;
                       onResize();
                   }
               });
