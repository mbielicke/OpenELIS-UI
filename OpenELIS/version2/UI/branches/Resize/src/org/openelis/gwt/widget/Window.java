...
import org.openelis.gwt.messages.Messages;
...
public class Window extends FocusPanel implements WindowInt, RequiresResize, ProvidesResize {
    ...
    @UiField
    protected LayoutPanel              outer, top, bottom;
    ...
    public Window() {
        ...
        resizer.addMouseDownHandler(new MouseDownHandler() {

            @Override
            public void onMouseDown(MouseDownEvent event) {
                resizeWindow.setWidth(getOffsetWidth() + "px");
                resizeWindow.setHeight(getOffsetHeight() + "px");
                RootPanel.get().add(resizeWindow, getAbsoluteLeft(), getAbsoluteTop());


                offX = (getAbsoluteLeft() + getOffsetWidth()) - event.getClientX();
                offY = (getAbsoluteTop() + getOffsetHeight()) - event.getClientY();
                RootPanel.get().addStyleName(css.NoSelect());
       ...
       resizeWindow.addMouseMoveHandler(new MouseMoveHandler() {
            public void onMouseMove(MouseMoveEvent event) {
                resizeWindow.setSize( (event.getClientX() - getAbsoluteLeft() + offX) +
                                     "px", (event.getClientY() - getAbsoluteTop() + offY) +
                                           "px");
            }
        });
        ...
        resizeWindow.addMouseUpHandler(new MouseUpHandler() {
            ...
            public void onMouseUp(MouseUpEvent event) {
                    RootPanel.get().removeStyleName(css.NoSelect());
                    outer.removeStyleName(css.NoSelect());
                    DOM.releaseCapture(resizeWindow.getElement());
                    setSize(resizeWindow.getOffsetWidth()-((css.borderWidth()*2)-2) + "px",
                            resizeWindow.getOffsetHeight()-((css.borderWidth()*2)-2) + "px");
                    RootPanel.get().remove(resizeWindow);
                    onResize();
            }
